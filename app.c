...
//      sl_app_log("System Boot\r\n");
      ...
      // set scan mode
      sc = sl_bt_scanner_set_mode(gap_1m_phy,
      ...
      // set scan timming
      sc = sl_bt_scanner_set_timing(gap_1m_phy,
                                    200,
                                    200);
      sl_app_assert(sc == SL_STATUS_OK,
...
//      08:ED:02:C7:00:07 , (remote_address->addr[5]<<40) | (remote_address->addr[4]<<32) |
      uint32_t temp =  (remote_address->addr[3]<<24) | (remote_address->addr[2]<<16) | (remote_address->addr[1]<<8) | remote_address->addr[0];
      if(!(temp ^ 0x02c70007)){
//      sl_app_log("\n\n##################################################:    %12.12x\r\n",temp);
